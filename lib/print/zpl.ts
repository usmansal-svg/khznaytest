...
export function zplLabel50x50(item: TagItem, opts: { thermalTransfer?: boolean; darkness?: number; copies?: number } = {}): string {
  ...
  // Generous margins: the ZD220 centres the label under its 4-inch head, so a label a millimetre narrower than 50 mm loses the outer edge.
  const inset = dots(3.5);
  const left = inset;
  const price = rs(item.list_price);
  const qrMag = 6; // 21 × 6 dots ≈ 15.8 mm
  const qrX = W - 21 * qrMag - inset, qrY = dots(3);
  const textW = qrX - left - dots(2);
  const priceW = Math.min(W - 2 * inset, Math.round(price.length * 23 + 36));
  const typeLine = item.is_rare ? `★ RARE FIND${item.rare_tag_line ? ` · ${item.rare_tag_line}` : ""}` : item.sub_category;
  return [
    "^XA", "^CI28", "^MNW", `^PW${W}`, `^LL${H}`, "^LH0,0", opts.thermalTransfer ? "^MTT" : "^MTD", opts.darkness != null ? `^MD${opts.darkness}` : "",
    `^FO${left},${dots(3)}^A0N,22,24^FDKHAZANAY^FS`,
    `^FO${left},${dots(6.6)}^A0N,34,34^FB${textW},1,0,L^FD${esc(cut(item.brand || "Unbranded", 16))}^FS`,
    `^FO${left},${dots(11.2)}^A0N,22,22^FB${textW},2,0,L^FD${esc(cut(typeLine, 40))}^FS`,
    `^FO${qrX},${qrY}^BQN,2,${qrMag}^FDQA,${esc(item.sku)}^FS`,
    `^FO${left},${dots(22.5)}^A0N,20,20^FDSIZE^FS`,
