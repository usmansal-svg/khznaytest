...
export type DbSubCategory = {
  ...
  /** Children's bands: offered to girls only, boys only, or any. */
  forWearer: "any" | "girls" | "boys";
...
async function readPricingContext(supabase: SupabaseClient): Promise<PricingContext | { failed: string }> {
  const [settingsRes, gradesRes, profilesRes, subsRes] = await Promise.all([
    ...
    supabase
      .from("sub_categories")
      .select("slug, code, category_slug, gender, name, weight_kg, profile_code, value_index, measure_type, season, market_ceiling, market_price, per_piece_cost, per_piece_share, standard_cost_pkr, heavy_cost_pkr, affordable_luxury_multiplier, al_premium_pkr, for_wearer, active"),
  ]);
  ...
  const subCategories: DbSubCategory[] = subsRes.data.map((s) => ({
    ...
    forWearer: (s.for_wearer === "girls" || s.for_wearer === "boys" ? s.for_wearer : "any") as "any" | "girls" | "boys",
