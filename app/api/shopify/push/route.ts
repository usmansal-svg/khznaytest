...
export async function POST(request: Request) {
  ...
  const { data: item, error } = await supabase
    .from("items")
    .select("id, sku, brand_text, brand_tier, grade_code, is_rare, season, wearer, size_label, colour, fabric, measurements, price, price_manual, status, photos, description, shopify_product_id, sub_categories(name, categories(name))")
    .eq("sku", sku)
    .maybeSingle();
  ...
  try {
    ...
    const input = {
      ...
      descriptionHtml: describe(item.description, item.measurements as Record<string, unknown>, item.grade_code, item.size_label),
...
function describe(text: string | null, measurements: Record<string, unknown>, grade: string, size: string | null): string {
  const parts: string[] = [];
