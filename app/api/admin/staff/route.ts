...
const ROLES = ["tagger", "qc_senior", "manager", "founder"];
...
export async function POST(request: Request) {
  ...
  if (!ROLES.includes(body.role ?? "")) return NextResponse.json({ error: "Role must be tagger, qc_senior, manager or founder." }, { status: 400 });
