...
const EDITABLE = ["category_slug", "gender", "name", "weight_kg", "profile_code", "value_index", "season", "market_ceiling", "market_price", "per_piece_cost", "per_piece_share", "planning_rate_usd_per_kg", "standard_cost_pkr", "heavy_cost_pkr", "affordable_luxury_multiplier", "al_premium_pkr", "active"] as const;
...
export async function GET() {
  ...
  const { data, error } = await supabase
    .from("sub_categories")
    .select("slug, code, category_slug, gender, name, weight_kg, profile_code, value_index, measure_type, season, market_ceiling, market_price, per_piece_cost, per_piece_share, planning_rate_usd_per_kg, standard_cost_pkr, heavy_cost_pkr, affordable_luxury_multiplier, al_premium_pkr, active, categories(name, sort_order)")
    .order("name");
...
type EstimateInput = { weight_kg: number; profile_code: string; value_index: number; planning_rate_usd_per_kg: number | null; per_piece_cost: number | null; standard_cost_pkr: number | null; market_price: number | null; affordable_luxury_multiplier?: number | null; al_premium_pkr?: number | null };
type Estimate = { landed_cost: number; loaded_cost: number; bnwt: number; premium: number; excellent: number; very_good: number; gp_pct: number; effective_gp_pct: number; al_multiplier: number; al_premium: number; al_premium_set: boolean };
...
export async function POST(request: Request) {
  ...
  let body: { rows?: { slug?: string; profile_code?: string; value_index?: number; standard_cost_pkr?: number | null; market_price?: number | null; affordable_luxury_multiplier?: number | null; al_premium_pkr?: number | null }[] };
  try {
  ...
  const { data: current } = await gate.db.from("sub_categories").select("slug, weight_kg, profile_code, value_index, planning_rate_usd_per_kg, per_piece_cost, standard_cost_pkr, market_price, affordable_luxury_multiplier, al_premium_pkr").in("slug", (body.rows ?? []).map((r) => r.slug ?? ""));
  ...
  for (const r of body.rows ?? []) {
    ...
    out[base.slug] = estimates({
      ...
      al_premium_pkr: opt(r.al_premium_pkr, base.al_premium_pkr == null ? null : Number(base.al_premium_pkr)),
    }, ctx);
  }
...
export async function PATCH(request: Request) {
  ...
  for (const row of body.rows) {
    ...
    const { data: before } = await supabase
      .from("sub_categories")
      .select("category_slug, gender, name, weight_kg, profile_code, value_index, season, market_ceiling, market_price, per_piece_cost, per_piece_share, planning_rate_usd_per_kg, standard_cost_pkr, heavy_cost_pkr, affordable_luxury_multiplier, al_premium_pkr, active")
      .eq("slug", slug)
      .maybeSingle();
    if (!before) {
...
function check(p: Record<string, unknown>): string | null {
  ...
  for (const k of ["market_ceiling", "market_price", "per_piece_cost", "planning_rate_usd_per_kg", "standard_cost_pkr", "heavy_cost_pkr", "affordable_luxury_multiplier", "al_premium_pkr"] as const) {
