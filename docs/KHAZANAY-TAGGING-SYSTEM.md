...
**Sleeves (13 Sep).** Sleeve length is an attribute, never a sub-category: every top-type garment (tops, outerwear, dresses, kids tops) asks *Half sleeve / Full sleeve / Sleeveless* on both forms, default Half sleeve, stored in `measurements.Sleeve` and sent to Shopify as a tag (`Half Sleeve`, `Full Sleeve`, `Sleeveless`) for filters and collections. Outerwear defaults to Full sleeve. **Measurements** (online form, flat, inches) follow the type, the gender and the sleeve: tops and outerwear Chest/Bust, Length; dresses Bust, Waist, Length; bottoms Waist, Length; kids tops Chest, Length; kids bottoms Waist, Length; plus **Sleeve length** whenever the sleeve is Full (`measurementFields()` in `lib/pricing/sub-categories.ts`). Women get "Bust" where men get "Chest". The long-sleeve sub-categories (Long sleeve T-shirt / polo / sports top, the infant bodysuit sleeve variants) were removed.
...
**Receiving is not flooring.** A received garment sits in the outlet's **stockroom** until the **POS** scans it onto the floor (the Floor stock screen moved to the POS on 13 Sep; its code is in `archive/pos/floor/`). That scan stamps `floored_on` (Pakistan date), the month's colour and who floored it, and moves the garment to *On floor · outlet*. A garment received on 1 January and floored on 5 January is floored on 5 January; one floored on 1 February gets February's colour. Only received garments can be floored (anything in transit is refused with the reason). *Floor the whole stockroom* does drop day in one go. Managers can *Upload … to Shopify POS* from a received transfer (its checked-in garments): tracked, one unit, at that outlet's Shopify location only.
...
## Screen permissions (14 Sep 2026)

Every screen is a permission (`lib/auth/permissions.ts`): Dashboard, Tag item, Items, Photos, Transfers, QC, Lots, Catalogue, Pricing, Brands, Scorecard, Settings. Each role has a default set (tagger: Tag, Items, Photos; photographer: Photos; QC senior adds Transfers and QC; manager and founder: everything; cashier: Items; outlet manager: Items and Transfers). On Settings → Staff a person can be given their own list of screens, which replaces the role's default (`staff.permissions`; null = default). The signed cookie carries the list, so a change applies at the person's next sign-in. The proxy maps every page and API path to a screen and blocks the rest; the menu shows only permitted screens, and sign-in lands on the first of them. Reference data, auth, tag images and a single garment's record stay open to everyone signed in.

## One name, two seasons (14 Sep 2026)

A sub-category name may exist once per gender **per season**. A row marked "all" seasons takes both slots. When a garment is really two garments (a summer Midi dress and a winter Midi dress, at different costs), adding the name again on the Catalogue screen offers a one-tap split: the existing row keeps one season and a new row is created for the other, with its numbers copied from the original (own slug, own SKU code, e.g. `women-midi-dress-winter`). The tag form only ever shows the rows for the season chosen, so the tagger sees one "Midi dress". Website tags are the same for both; the season tags tell them apart. Changing a row's season is refused if it would clash with its twin.
