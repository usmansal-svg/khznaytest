...
**Sleeves (13 Sep).** Sleeve length is an attribute, never a sub-category: every top-type garment (tops, outerwear, dresses, kids tops) asks *Half sleeve / Full sleeve / Sleeveless* on both forms, default Half sleeve, stored in `measurements.Sleeve` and sent to Shopify as a tag (`Half Sleeve`, `Full Sleeve`, `Sleeveless`) for filters and collections. Outerwear defaults to Full sleeve. **Measurements** (online form, flat, inches) follow the type, the gender and the sleeve: tops Chest/Bust, Length, Shoulder; outerwear the same; dresses Bust, Waist, Length; bottoms Waist, Length, Inseam; kids tops Chest, Length; kids bottoms Waist, Length; plus **Sleeve length** whenever the sleeve is Full (`measurementFields()` in `lib/pricing/sub-categories.ts`). Women get "Bust" where men get "Chest". The long-sleeve sub-categories (Long sleeve T-shirt / polo / sports top, the infant bodysuit sleeve variants) were removed.
