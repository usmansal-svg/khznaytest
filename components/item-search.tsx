                ...
                <tbody className="divide-y">
                  {visible.map((r) => (
                    <tr key={r.id} className={cn(picked.has(r.sku) && "bg-muted/40")}>
                      ...
                      <td className="py-2 pr-2"><span className={cn("rounded-full border px-2 py-0.5 text-xs", /^Received|Online shelf|^Sold$/.test(r.station) ? "border-green-600 text-green-700 dark:text-green-400" : /transit|dispatched|Packing/.test(r.station) ? "border-sky-500 text-sky-700 dark:text-sky-300" : /QC|Set aside|Pulled|damaged|Rejected|Unlisted/.test(r.station) ? "border-amber-500 text-amber-700 dark:text-amber-300" : "text-muted-foreground")}>{r.station}</span></td>
