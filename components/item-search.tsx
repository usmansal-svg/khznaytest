                ...
                <tbody className="divide-y">
                  {visible.map((r) => (
                    <tr key={r.id} className={cn(picked.has(r.sku) && "bg-muted/40")}>
                      ...
                      <td className="py-2 pr-2 text-xs">{r.shopify ? <span className={cn("rounded-full border px-2 py-0.5", r.shopify_error ? "border-red-400 text-red-700 dark:text-red-300" : r.shopify === "draft" ? "text-muted-foreground" : "border-sky-500 text-sky-700 dark:text-sky-300")} title={r.shopify_error ?? undefined}>{SHOPIFY_LABEL[r.shopify] ?? r.shopify}{r.shopify_error ? " ⚠" : ""}</span> : <span className="text-muted-foreground">—</span>}</td>
