...
type Est = { landed_cost: number; loaded_cost: number; bnwt: number; premium: number; excellent: number; very_good: number; gp_pct: number; effective_gp_pct: number; al_multiplier?: number; al_premium?: number; al_premium_set?: boolean };
type SubRow = { slug: string; code: string; name: string; gender: string; category_slug: string; category: string; weight_kg: number; profile_code: string; value_index: number; season: "summer" | "winter" | "all"; market_ceiling: number | null; market_price: number | null; standard_cost_pkr: number | null; heavy_cost_pkr: number | null; affordable_luxury_multiplier: number | null; al_premium_pkr: number | null; active: boolean; estimate?: Est | null };
  ...
  return (
    ...
    <Card>
      ...
      <CardContent>
        ...
        <div className="mb-3 flex flex-wrap items-center gap-x-4 gap-y-2 rounded-lg border bg-muted/40 px-3 py-2 text-xs">
          <span className="font-semibold uppercase tracking-wide text-muted-foreground">Filter</span>
          <span className="flex items-center gap-1 rounded-md border bg-background px-2 py-1"><HeaderFilter label="Gender" values={genderValues} selected={filters.gender} onChange={(v) => setFilters((f) => ({ ...f, gender: v }))} format={(g) => GENDER_OPTIONS.find((o) => o.code === g)?.name ?? g} /></span>
          <span className="flex items-center gap-1 rounded-md border bg-background px-2 py-1"><HeaderFilter label="Category" values={categoryValues} selected={filters.category} onChange={(v) => setFilters((f) => ({ ...f, category: v }))} /></span>
          <span className="flex items-center gap-1 rounded-md border bg-background px-2 py-1"><HeaderFilter label="Sub-category" values={nameValues} selected={filters.name} onChange={(v) => setFilters((f) => ({ ...f, name: v }))} /></span>
          ...
          <table className="w-full text-xs">
            <thead className="sticky top-0 z-10 bg-background text-left text-[11px] uppercase text-muted-foreground shadow-[0_1px_0_0_hsl(var(--border))]">
              <tr>
                ...
                <th className="pb-2 pl-3 pr-3 text-right" title="Premium price for an affordable-luxury brand, Rs. Grey = calculated from the AL multiple; type a figure to set it by hand (blue border).">AL Premium</th>
              ...
              {visible.map((r, idx) => {
                ...
                return (
                  ...
                  <tr className={cn("hover:bg-muted/30", !v.active && "opacity-50")}>
                    ...
                    <td className="py-1 pl-3 pr-3"><Input type="number" step="10" min="0" value={v.al_premium_pkr ?? ""} placeholder={est?.al_premium != null ? n0(est.al_premium) : "—"} title="Premium price for an affordable-luxury brand. Blank = the AL multiple applied; type a price to set it by hand." onChange={(ev) => edit(r.slug, { al_premium_pkr: ev.target.value === "" ? null : Number(ev.target.value) })} className={cn("h-7 w-20 px-1 text-right text-xs font-semibold", changed("al_premium_pkr") && "border-amber-500", v.al_premium_pkr != null && "border-sky-500")} /></td>
