                  ...
                  <tr key={r.slug} className={cn(!v.active && "opacity-50")}>
                    ...
                    <td className="py-1.5 pr-2">
                      <div className="flex items-center gap-1">
                        <Input type="number" step="10" min="1" value={v.heavy_cost_pkr ?? ""} placeholder="—" onChange={(ev) => edit(r.slug, { heavy_cost_pkr: ev.target.value === "" ? null : Number(ev.target.value) })} className={cn("h-8 w-24", changed("heavy_cost_pkr") && "border-amber-500")} />
                        {v.heavy_cost_pkr == null
                          ? <button type="button" title="Set the heavy cost to 30% above the cost per piece — the tag form then asks Regular or Heavy for this garment" disabled={!v.standard_cost_pkr} onClick={() => edit(r.slug, { heavy_cost_pkr: Math.round((Number(v.standard_cost_pkr) * 1.3) / 10) * 10 })} className="rounded border px-1.5 py-0.5 text-[11px] text-muted-foreground hover:bg-muted disabled:opacity-40">+30%</button>
                          : <button type="button" title="No heavy version: the tag form stops asking" onClick={() => edit(r.slug, { heavy_cost_pkr: null })} className="rounded px-1 text-[11px] text-muted-foreground hover:bg-muted">×</button>}
                      </div>
                    </td>
                    <td className="py-1.5 pr-2">
