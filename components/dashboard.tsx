            ...
            <div className="overflow-x-auto"><table className="w-full text-sm">
              ...
              <tbody className="divide-y">{data.transfers.map((t) => (<>
                <tr key={t.id} className="cursor-pointer hover:bg-muted/50" onClick={() => setOpenTransfer(openTransfer === t.id ? null : t.id)}>
                  ...
                  <td className="py-1.5"><span className={cn("rounded-full border px-2 py-0.5 text-xs capitalize", t.status === "received" && "border-green-600 text-green-700 dark:text-green-400", (t.status === "dispatched" || t.status === "receiving") && "border-amber-600 text-amber-700 dark:text-amber-400")}>{{ packing: "Packing", dispatched: "In transit", receiving: "Receiving", received: "Received" }[t.status as string] ?? t.status}</span></td>
